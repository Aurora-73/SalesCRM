...
改进前的 MCP 系统暴露了 53 个工具，完整覆盖功能域，但纯 MCP 使用者（Claude Desktop / Cursor / Windsurf）面临"零件箱，没有组装说明"的问题——工具列表是 API 目录，不是操作手册。
...
通过新增 `guide` 工具（11 个主题）+ 增强 23 个关键工具的描述，解决全部 15 项信息差。详见 [mcp_guide_completion.md](file:///E:/Code/SalesCRM/readme/mcp_guide_completion.md)。
...
| 2 | 把公式当评分系统 → 机械套阈值 | 方法论层缺失 | 15 个公式描述加"【辅助参考·核验而非套用】"；guide("methodology") | ✅ |
...
| 公式的定位 | formula_calc_* / sales_calc_* 写了"辅助参考" | 公式数值**只在数据全貌表出现一次**，策略全部回指 Wiki |
| 核验而非套用 | 看不到 | 读公式结果→结合 Wiki 知识核验→自己做判断。BQ=0.5 不一定比 BQ=1.1 差 |
| 操作顺序 | 看不到 | ① Wiki → ② 事实档案 → ③ 实时数据 → ④ 公式核验 |
| 两套公式系统 | 看不到 | 战态分析（IVI/SPE/EWS）+ 销售决策（BQ/BSP/BWS/PV），均作辅助参考 |
...
SalesCRM 独有的两套公式系统（战态分析 IVI/SPE/EWS + 销售决策 BQ/BSP/BWS/PV）增加了信息差风险：
- Agent 可能不知道该用哪套公式
- 两套公式的阈值不同（IVI 阈值 vs BQ 阈值）
- guide("reference/formula") 已明确说明两套公式的用途和阈值
