...
用法：
    python tools/label_stickers.py generate [--limit 100]  # 生成 HTML 标注页面
    python tools/label_stickers.py import                  # 从 JSON 导入标注结果
"""
...
from engine.stickers import ensure_stickers_table, label_sticker
...
stickers.forEach((s, i) => {{
  ...
  card.innerHTML = `
    ...
    <select onchange="updateEmotion(${{i}}, this.value)">
      ...
      <option value="positive">positive (积极/亲近/认同)</option>
      <option value="negative">negative (消极/拒绝/敷衍)</option>
      <option value="neutral">neutral (中性，不表达态度)</option>
      <option value="ambiguous">ambiguous (试探/模糊)</option>
    </select>
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法:")
        print("  python tools/label_stickers.py generate [--limit 100] [--min-freq 1]")
        print("  python tools/label_stickers.py import")
        sys.exit(1)
