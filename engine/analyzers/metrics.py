...
从 SQLite 读取消息数据，计算 16 个 MetricValue 加权指标 + signal_flags（动态信号）+ media_engagement（媒体互动）+ neediness_penalty（跟进投入惩罚）+ interaction_pattern（互动模式标签）。
...
def compute_fback(conn, my_wxid, contact_wxid, window_days=30):
...
def compute_rlatency(conn, my_wxid, contact_wxid, session_gap_hours=4):
    rows = conn.execute(
...
def _compute_customer_ratio(conn, contact_wxid, char, window_days):
...
def _compute_emotion_ratio(conn, my_wxid, contact_wxid, window_days):
    ...
    def _has_unicode_emoji(content):
...
def compute_moments(conn, contact_wxid):
    rows = conn.execute(
...
def compute_msg_count(conn, contact_wxid):
    rows = conn.execute(
...
def compute_active_days(conn, contact_wxid, window_days=30):
    cutoff = int((datetime.now() - timedelta(days=window_days)).timestamp())
    rows = conn.execute(
...
def compute_recent(conn, contact_wxid, recency_decay=90):
    rows = conn.execute(
...
def compute_base_score(metrics, weights, top_target=False):
...
def compute_signal_level(composite):
    if composite >= 0.70:
        return "强意向"
    if composite >= 0.50:
        return "中意向"
...
def compute_fback_quality(conn, my_wxid, contact_wxid, window_days=30):
...
def compute_escore_volatility(conn, my_wxid, contact_wxid, window_days=30, session_gap_hours=4):
...
def compute_qscore_detailed(conn, contact_wxid, window_days=30):
...
def compute_rlatency_context(conn, my_wxid, contact_wxid, window_days=30, session_gap_hours=4):
    """慢回时有解释的比例。有解释 = 正向信号。"""
    cutoff = int((datetime.now() - timedelta(days=window_days)).timestamp())
    rows = conn.execute(
        "SELECT sender_id, content, timestamp FROM messages "
...
def compute_neediness_penalty(conn, my_wxid, contact_wxid, session_gap_hours=4):
...
def compute_msg_volume_trend(conn, contact_wxid, window_days=30):
...
def compute_latency_trend(conn, my_wxid, contact_wxid, window_days=30, session_gap_hours=4):
...
def compute_media_engagement(conn, contact_wxid, my_wxid, window_days=30):
...
def compute_session_recency(conn, contact_wxid, session_gap_hours=4):
...
def compute_momentum(conn, contact_wxid, my_wxid, session_gap_hours=4):
...
def compute_initiation_source(conn, contact_wxid, my_wxid, session_gap_hours=4):
...
def compute_metrics_for_contact(conn, config, contact_wxid, contact_name="",
                                top_target=False, prev_base_score=None):
    """为单个联系人计算全部指标。"""
...
def compute_sales_metrics(conn, my_wxid, contact_wxid, window_days=90):
    """计算销售特有指标。"""
    cutoff = int((datetime.now() - timedelta(days=window_days)).timestamp())
    rows = conn.execute(
        "SELECT content FROM messages "
...
def get_all_contacts_with_messages(conn, min_messages=0):
    """获取所有有消息的联系人（私聊）。"""
    rows = conn.execute(
        """
        SELECT c.id AS wxid,
